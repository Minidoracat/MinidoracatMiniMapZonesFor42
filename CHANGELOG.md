...
- **MP 重登後整場看不到自訂區域**（正式服實證）：進場的全量請求原本只在
  OnGameStart 送一次——該時點 MP 連線未必就緒（onlineId 尚未指派、GameClient.ingame
  尚未 true），sendClientCommand 可能靜默丟失；而既有補發機制只在「收到過部分
  分包」時觸發，初始請求丟失＝永不補發、整場空區域。生成範本／/reloadzones 當下
  看得到（走伺服器主動廣播）、重登即消失（走此請求）正是此因。改為 OnTick 等
  連線就緒（getOnlineID）後送出、每 10 秒重送直到收到伺服器回應（上限 12 次；
  同 process 重登會重置重試狀態），沿主 MOD SteamIdReport 同款先例。
  純客戶端修正，與任何版本的伺服器端相容。

### 調整

- **移除「顯示伺服器區域」母開關**：與主 MOD「顯示自訂區域圖層」總開關作用完全
  重疊（本包是家族唯一 zone provider，兩顆相鄰的等效開關只造成困惑——實測回饋）。
  註冊時不再傳 optionLabelKey，主 MOD 不再生成這顆開關；區域顯示改由總開關單獨
  控制。先前關過此開關的玩家升級後區域會重新出現（舊設定值失效屬預期），用
  「顯示自訂區域圖層」關回即可。主 MOD 的 per-provider 開關機制保留（未來第三方
  區域 addon 仍可用）。對主 MOD 版本無新要求。

## [42.20.1-0.4.0] - 2026-08-14

### 新功能

- **`haloAlpha` 暗色描邊欄位**：zones.json 各區域可加 `haloAlpha`（0–1）——
  區塊底下墊一層外擴的暗色底取代框線，邊界一樣清楚、繪製成本更低
  （主 MOD 資源點區塊同款視覺）。與 `border`／`borderAlpha` 並存可疊用
- **建物尺度區域自動縮放 LOD**：區域矩形聯集最長邊 ≤ 100 格者自動附掛
  LOD 外框——地圖拉遠到中距只畫一塊聯集色塊、更遠整組隱藏，拉近才畫
  細節與名稱；大量建物尺度區域（店家標記等）在全圖視野下不再逐矩形硬畫。
  城鎮尺度大區域不受影響、照常恆顯
- **範本帶類別演示**：生成的示範區域改為「示範：城鎮」×3＋「示範：野外」×1
  兩個 `category`——搭配主 MOD 0.14.0 的「伺服器區域」設定區，玩家開窗
  就能看到類別勾選篩選怎麼運作（四語範本同步）

### 配套（主 MOD 42.20.1-0.14.0）

- 主 MOD 統一視窗新增「伺服器區域」區：依 zones.json 實際 `category` 動態
  生成逐類勾選（全選／全不選）、「區域名稱不受縮放限制」開關（預設開）、
  範本生成列移入此區；**視窗開著時區域資料到貨自動刷新**——按下生成或
  `/reloadzones` 後類別勾選即時出現
- 本版新欄位在舊版主 MOD 下**安靜降級**：`haloAlpha` 被忽略（無描邊）、
  LOD 不生效（照常恆顯）、類別照常過驗證但無篩選 UI——不會壞，只是
  看不到新效果；要完整效果請把主 MOD 更新到 42.20.1-0.14.0 以上

> 技術要點：lodRect 是「計算欄位」（attachLodRect 於驗證/解包時就地附掛、
> 每 zone 一次非每幀；SP validateZones 與 MP unpackZone 兩路都過）——刻意
> 不進 wire（省 bytes），client 端解包後重算；列入 KNOWN_FIELDS 使手寫
> 頂層 lodRect 被靜默丟棄而非塞進 meta 占 wire（codex review 抓出的窄例外）。
> haloAlpha wire 短欄位名 `ha`。LOD_MAX_EDGE=100。測試 103 案例維持全綠、
> 範本四語常數 test_tpl_strings.py 逐字鎖定。
