...
[list]
[*] [b]サーバー定義ゾーン[/b]：サーバー側の `zones.txt`（外部ツールから書き込み可能——イベント範囲ツールやリセットゾーンマーカーツールなど）。サーバーが検証してから全プレイヤーへリアルタイム配信；シングルプレイはローカルの `zones.txt` を直接読み込みます
[*] [b]42.20 ファイル名移行（0.2.0）[/b]：ゲーム 42.20 で Lua ファイル書き込みに拡張子ホワイトリストが導入されたため、ファイルは `zones.txt` になりました（内容は引き続き JSON 形式）。既存の `zones.json` は初回起動時に自動で `zones.txt` へ移行されます（一回限り）。外部ツールは今後 `zones.txt` に書き込んでください
[/list]
...
[*] [b]サーバー権威同期[/b]：`zones.txt` を編集するとポーリング間隔内（サンドボックスで調整可）で自動更新。管理者は [b]/reloadzones[/b] コマンドで即座に強制更新も可能。途中参加したプレイヤーも現在のゾーンをすぐ確認できます
...
[*] [b]自動テンプレート＆生成ボタン[/b]：初回起動時に 4 つのデモゾーン入り `zones.txt` テンプレートを自動生成（サーバー言語に合わせてローカライズ）。統合設定ウィンドウには言語選択（繁体字中国語／簡体字中国語／英語／日本語）付きの「ゾーンテンプレートを生成」ボタンもあり——確認ダイアログで誤操作を防ぎ、既存ファイルはタイムスタンプ付き `.bak.txt` にバックアップしてから上書き。サーバーでは「MOD 管理」権限が必要です
...
[*] [b]シングル／マルチ両対応[/b]：シングルプレイはローカルの `zones.txt` を直接読み込み、マルチプレイはサーバーが検証・配信
[/list]
