...
[*] [b]カテゴリ別表示切替（0.4.0）[/b]：ゾーンに `category` フィールドを付与——本体 MOD の「サーバーゾーン」設定セクションにカテゴリ別チェックボックス（全選択／全解除付き）が自動生成され、ウィンドウを開いたままでもデータ到着時に自動更新；テンプレートは「デモ：町／デモ：野外」でその場で体験可
