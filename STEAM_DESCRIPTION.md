...
[list]
[*] [b]伺服器自訂區域[/b]：伺服器端 `zones.txt` 定義（可由外部程式寫入，例如活動範圍工具、重置區標記工具），伺服器驗證後即時廣播給所有玩家；單機直接讀取本地 `zones.txt`
[*] [b]42.20 檔名遷移（0.2.0）[/b]：遊戲 42.20 起 Lua 寫檔有副檔名白名單，檔案由 `zones.json` 改為 `zones.txt`（內容仍為 JSON 格式）；舊 `zones.json` 首次啟動自動搬入 `zones.txt`（一次性），外部程式請改寫 `zones.txt`
[/list]
...
[*] [b]伺服器權威同步[/b]：改 `zones.txt` 後，輪詢間隔內（sandbox 可調）自動更新；管理員也可用 [b]/reloadzones[/b] 指令立即刷新；新進玩家進服即看到當前區域
...
[*] [b]自動範本與生成按鈕[/b]：首次啟動自動生成含四個示範區域的 `zones.txt` 範本（依伺服器語系出字）；統一設定視窗另有「生成區域範本」按鈕，可選語言（繁中／簡中／英／日）重生範本——確認視窗防誤按，現有檔先備份為帶時間戳的 `.bak.txt` 再覆寫，伺服器上需「管理模組」權限
[*] [b]enabled 欄位[/b]：區域可設 `"enabled": false` 保留座標但暫時隱藏，不必刪除
[*] [b]壞資料容錯[/b]：格式錯誤或超出上限的條目會被跳過並記錄，不影響其他區域正常顯示
[*] [b]單機／多人皆可用[/b]：單機直接讀取本地 `zones.txt`；多人由伺服器統一驗證與廣播
[/list]
